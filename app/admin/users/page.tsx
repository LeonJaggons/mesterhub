...
export default function AdminUsersPage() {
  ...
  const { items: users, loading, error, setError, load: loadUsers } = useAdminList<AdminUser>('/api/admin/users', 'users')

  useEffect(() => {
    void loadUsers({ q: query })
  ...
  return (
    <>
      <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <form onSubmit={search} className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end">
          <AdminField label="Search" className="flex-1">
            <AdminInput value={query} onChange={e => setQuery(e.target.value)} placeholder="Name, email, phone, UID, district" />
          </AdminField>
      ...
      <AdminListState loading={loading} empty={users.length === 0} emptyMessage="No customers found." gridClassName="grid grid-cols-1 gap-5 lg:grid-cols-2">
        {users.map(user => (
          <AdminCard key={user.uid}>
            <AdminCardHeader
              title={user.displayName || 'Unnamed customer'}
              subtitle={`${user.email || 'No email'} · ${user.uid}`}
              badge={(
