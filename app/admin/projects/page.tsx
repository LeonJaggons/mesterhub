  ...
  return (
    <>
      <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 lg:grid-cols-4">
          <label className="text-sm font-semibold text-gray-700">
            Status
            <select value={status} onChange={e => { setStatus(e.target.value); loadProjects(e.target.value, query) }} className="mt-1 w-full rounded-md border border-gray-200 bg-white px-3 py-2 text-gray-900">
          ...
          <label className="text-sm font-semibold text-gray-700 lg:col-span-2">
            Search
            <input value={query} onChange={e => setQuery(e.target.value)} onBlur={() => loadProjects(status, query)} placeholder="Name, email, category, district, or UID" className="mt-1 w-full rounded-md border border-gray-200 px-3 py-2 text-gray-900" />
          </label>
          <label className="text-sm font-semibold text-gray-700">
            Admin reason
            <input value={reason} onChange={e => setReason(e.target.value)} placeholder="Optional reason" className="mt-1 w-full rounded-md border border-gray-200 px-3 py-2 text-gray-900" />
          </label>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center text-gray-500">No projects match these filters.</div>
      ) : (
